...
def _load_sounds() -> Dict[str, Optional[arcade.Sound]]:
    """Load all configured sounds and start background music if available."""

    sounds: Dict[str, Optional[arcade.Sound]] = {}
    for key, relative_path in settings.SOUND_FILES.items():
        full_path = os.path.join(settings.BASE_DIR, relative_path)
        if os.path.exists(full_path):
            print(f"{os.path.basename(full_path)} found")
            sounds[key] = arcade.load_sound(full_path)
        else:
            print(f"{os.path.basename(full_path)} missing")
            sounds[key] = None

    music = sounds.get("music")
    if music:
        arcade.play_sound(music, volume=0.3)
...
class StickmanFighterGame(arcade.Window):
    ...
    def __init__(self) -> None:
        super().__init__(settings.WIDTH, settings.HEIGHT, "Der Kampf des Zwei Könige")
        ...
        self.controls1 = {"left": settings.KEY.A, "right": settings.KEY.D, "jump": settings.KEY.W,
                          "punch": settings.KEY.F, "kick": settings.KEY.G}
        self.controls2 = {"left": settings.KEY.LEFT, "right": settings.KEY.RIGHT, "jump": settings.KEY.UP,
                          "punch": settings.KEY.NUM_0, "kick": settings.KEY.NUM_1}

        fighter1_cfg = settings.FIGHTER_OVERRIDES["fighter1"]
        fighter2_cfg = settings.FIGHTER_OVERRIDES["fighter2"]

        fighter1_path = os.path.join(settings.BASE_DIR, settings.FIGHTER_SPRITE_DIRS["fighter1"])
        fighter2_path = os.path.join(settings.BASE_DIR, settings.FIGHTER_SPRITE_DIRS["fighter2"])
    ...
    def load_background(self, mode: str) -> None:
        ...
        background_path = os.path.join(settings.BASE_DIR, "assets", "image_files", bg_filename)
    ...
    def draw_hud(self) -> None:
        arcade.draw_lrtb_rectangle_filled(left=0, right=settings.WIDTH, top=settings.HEIGHT,
                                          bottom=settings.HEIGHT - 80, color=settings.HUD_BG)
        ...
        x1_left = pad
        x1_right = pad + bar_w
        arcade.draw_lrtb_rectangle_filled(left=x1_left, right=x1_right, top=top_y + 10,
                                          bottom=top_y - 10, color=settings.RED)
        f1_green_w = (max(0, self.fighter1.health) / 100) * bar_w
        arcade.draw_lrtb_rectangle_filled(left=x1_left, right=x1_left + f1_green_w,
                                          top=top_y + 10, bottom=top_y - 10, color=settings.GREEN)
        ...
        arcade.draw_lrtb_rectangle_filled(left=x2_left, right=x2_right, top=top_y + 10,
                                          bottom=top_y - 10, color=settings.RED)
        f2_green_w = (max(0, self.fighter2.health) / 100) * bar_w
        arcade.draw_lrtb_rectangle_filled(left=x2_right - f2_green_w, right=x2_right,
                                          top=top_y + 10, bottom=top_y - 10, color=settings.GREEN)
        ...
        if self.mode:
            ...
            if mode_text:
                arcade.draw_text(mode_text, settings.WIDTH / 2, settings.HEIGHT - 60,
                                 settings.WHITE, 14, anchor_x="center")
    ...
    def on_draw(self) -> None:
        ...
        if self.background:
            ...
            arcade.draw_texture_rectangle(settings.WIDTH // 2 + offset_x, settings.HEIGHT // 2,
                                          settings.WIDTH, settings.HEIGHT, self.background)
        else:
            arcade.draw_lrtb_rectangle_filled(left=0, right=settings.WIDTH, top=200,
                                              bottom=0, color=settings.GROUND)
        ...
        if self.state == "round_over":
            arcade.draw_text(f"{self.winner} hat die Runde gewonnen!", settings.WIDTH / 2,
                             settings.HEIGHT / 2 + 40, settings.WHITE, 36, anchor_x="center", bold=True)
            arcade.draw_text("Nächste Runde...", settings.WIDTH / 2, settings.HEIGHT / 2 - 5,
                             settings.WHITE, 18, anchor_x="center")
        elif self.state == "match_over":
            arcade.draw_text(f"{self.winner} GEWINNT DAS DUELL!", settings.WIDTH / 2,
                             settings.HEIGHT / 2 + 40, settings.WHITE, 40, anchor_x="center", bold=True)
            arcade.draw_text("Zurück zum Menue...  (R = Wiederspielen, M = Menue)",
                             settings.WIDTH / 2, settings.HEIGHT / 2 - 5, settings.WHITE, 18, anchor_x="center")
    ...
    def _draw_menu(self) -> None:
        arcade.draw_lrtb_rectangle_filled(0, settings.WIDTH, settings.HEIGHT, 0, (20, 20, 20))
        arcade.draw_text("Der Kampf des Zwei Könige", settings.WIDTH / 2, settings.HEIGHT - 140,
                         settings.WHITE, 48, anchor_x="center", bold=True)
        ...
        for index, label in enumerate(button_labels):
            ...
            arcade.draw_lrtb_rectangle_filled(x, x + btn_w, y + btn_h, y, (50, 50, 50))
            arcade.draw_lrtb_rectangle_outline(x, x + btn_w, y + btn_h, y, (200, 200, 200), 3)
            arcade.draw_text(label, x + btn_w / 2, y + btn_h / 2, settings.WHITE, 24,
                             anchor_x="center", anchor_y="center")

        current_mode = {"day": "TAG", "night": "NACHT"}.get(self.mode or "", "NICHT GEWÄHLT")
        mode_label = f"Modus: {current_mode}"
        arcade.draw_text(mode_label, settings.WIDTH / 2, settings.HEIGHT / 2 - 180,
                         (200, 200, 200), 16, anchor_x="center")
        arcade.draw_text("Klicke auf einen Block zum Auswählen", settings.WIDTH / 2,
                         settings.HEIGHT / 2 - 210, (180, 180, 180), 12, anchor_x="center")

    def _draw_options(self) -> None:
        arcade.draw_lrtb_rectangle_filled(0, settings.WIDTH, settings.HEIGHT, 0, (15, 15, 15))
        arcade.draw_text("Optionen", settings.WIDTH / 2, settings.HEIGHT - 120,
                         settings.WHITE, 40, anchor_x="center", bold=True)
        ...
        for label, x in [("TAG", day_x), ("NACHT", night_x)]:
            arcade.draw_lrtb_rectangle_filled(x, x + btn_w, y + btn_h, y, (60, 60, 60))
            arcade.draw_lrtb_rectangle_outline(x, x + btn_w, y + btn_h, y, (220, 220, 220), 3)
            arcade.draw_text(label, x + btn_w / 2, y + btn_h / 2, settings.WHITE, 26,
                             anchor_x="center", anchor_y="center")

        arcade.draw_text("Wähle die Arena-Beleuchtung. Nach der Auswahl kehrst du zum Menue zurück.",
                         settings.WIDTH / 2, settings.HEIGHT / 2 - 120,
                         (200, 200, 200), 14, anchor_x="center")
        arcade.draw_text("ESC zum Beenden, M fuer Menue", settings.WIDTH / 2, settings.HEIGHT / 2 - 150,
                         (180, 180, 180), 12, anchor_x="center")
